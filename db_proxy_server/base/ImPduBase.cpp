...
CImPdu* CImPdu::ReadPdu(Buffer* buf)
{
	//typedef struct {
	...
	uint32_t pdu_len = 0;
	if (!IsPduAvailable(buf, buf->readableBytes(), pdu_len))  //验证包是否完整。
		return NULL;
	/*uint16_t service_id = buf->readint16();
	uint16_t command_id = buf->readint16();*/	//modify:在这里有什么作用？？？
	CImPdu* pPdu = new CImPdu();
	vector<uchar_t> vec;
	for (int i = 0; i < pdu_len; ++i)
		vec.push_back( buf->readInt8());  //Buffer中的数据已在这里被读取
	
	pPdu->Write(vec.data(), pdu_len);
	pPdu->ReadPduHeader(vec.data(), IM_PDU_HEADER_LEN);  

	return pPdu;
}

bool CImPdu::IsPduAvailable(Buffer* buf, uint32_t len, uint32_t& pdu_len)
{
	if (len < IM_PDU_HEADER_LEN)
		return false;

	pdu_len = buf->peekInt32();   //Pdu的头部，前32位代表整个Pdu的长度。
	if (pdu_len > len)
	{
		log("pdu_len=%d, len=%d\n", pdu_len, len);
		return false;
