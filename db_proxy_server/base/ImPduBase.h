...
public:
	...
	static bool IsPduAvailable(Buffer* buf, uint32_t len, uint32_t& pdu_len);
	//static CImPdu* ReadPdu(uchar_t* buf, uint32_t len); //被修改成下式
	static CImPdu* ReadPdu(Buffer* buf);
	void Write(const uchar_t* buf, uint32_t len) { m_buf.Write((void*)buf, len); }
