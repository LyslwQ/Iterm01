...
#include "../../base/pb/protocol/IM.Group.pb.h"
...
void createGroup(CImPdu* pPdu, const TcpConnectionPtr& conn)
{
  ...
  if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength())){
      ...
      if(IM::BaseDefine::GroupType_IsValid(nGroupType)){ //群组类型： 1-固定； 2-临时
          ...
          log("createGroup.%d create %s, userCnt=%u, result:%d", 
               nUserId, strGroupName.c_str(), setMember.size(), msgResp.result_code());
  	  ...
  	  conn->send(pPduRes->GetBuffer(), pPduRes->GetLength());  //数据最终会在conn所在的io线程中发送出去。
      }
      else
          log("invalid group type.userId=%u, groupType=%u, groupName=%s", 
...
void modifyMember(CImPdu* pPdu, const TcpConnectionPtr& conn)
{
  ...
  if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength())){
      ...
      if (IM::BaseDefine::GroupModifyType_IsValid(nType) &&
          ...
          log("userId=%u, groupId=%u, result=%u, changeCount:%u, currentCount=%u",
               nUserId, nGroupId,  bRet?0:1, msgResp.chg_user_id_list_size(), msgResp.cur_user_id_list_size());
          msgResp.set_attach_data(msg.attach_data());
          pPduRes->SetPBMsg(&msgResp);
          pPduRes->SetSeqNum(pPdu->GetSeqNum());
          pPduRes->SetServiceId(IM::BaseDefine::SID_GROUP);
          ...
          log("invalid groupModifyType or groupId. userId=%u, groupId=%u, groupModifyType=%u", 
