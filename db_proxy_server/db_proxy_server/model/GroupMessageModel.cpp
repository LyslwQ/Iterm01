...
{
  bool bRet = false;
  if(CGroupModel::getInstance()->isInGroup(nFromId, nGroupId)){ //上一层已经验证过了，这里还需要？？？
      ...
      if (pDBConn){
          ...
          // 在muduo库的网络模型下，sendMessage可以被多个loop调用，所以线程安全应该由GetMysql()去保证。
          ...
          if (pStmt->Init(pDBConn->GetMysql(), strSql)){
              ...
              if (bRet){
                  CGroupModel::getInstance()->updateGroupChat(nGroupId);  //更新聊天时间
    ...
    if (pDBConn){
        ...
        if(nMsgId == 0){
            strSql = "select * from " + strTableName 
                     ...
                     + " and status = 0 and created>="+ int2string(nUpdated) 
                     + " order by created desc, id desc limit " + int2string(nMsgCnt);
        }
        else {
            strSql = "select * from " + strTableName 
                     + " where groupId = " + int2string(nGroupId) 
                     + " and msgId<=" + int2string(nMsgId) 
                     + " and status = 0 and created>="+ int2string(nUpdated) 
                     + " order by created desc, id desc limit " + int2string(nMsgCnt);
       }
...
void CGroupMessageModel::getUnreadMsgCount(uint32_t nUserId, uint32_t &nTotalCnt, list<IM::BaseDefine::UnreadInfo>& lsUnreadCount)
{
   ...
   if (pCacheConn){
       for(auto it=lsGroupId.begin(); it!=lsGroupId.end(); ++it){
           ...
           if(nGroupCnt >= nUserCnt) //群的消息总数-用户已读的消息总数 = 用户未读的群信息总数
           ...
           if(nCount > 0){
               ...
               if(IM::BaseDefine::MsgType_IsValid(nType)){
                   ...
                   log("invalid msgType. userId=%u, groupId=%u, msgType=%u, msgId=%u", 
                        nUserId, nGroupId, nType, nMsgId);
          }
      }
      pCacheManager->RelCacheConn(pCacheConn);
   }
...
void CGroupMessageModel::getLastMsg(uint32_t nGroupId, uint32_t &nMsgId, string &strMsgData, IM::BaseDefine::MsgType &nMsgType, uint32_t& nFromId)
{
  ...
  CDBConn* pDBConn = pDBManager->GetDBConn("teamtalk_slave");
  if (pDBConn){
      string strSql = "select msgId, type,userId, content from " + strTableName 
                      + " where groupId = " + int2string(nGroupId) 
      ...
      if (pResultSet){
          ...
          log("no result set for sql: %s", strSql.c_str());
      pDBManager->RelDBConn(pDBConn);  //该链接应该在获得结果后就释放？？？
  }
  else
      log("no db connection for teamtalk_slave");
}
