...
//@param :nUserId--请求者的id       ； nPeerId--对方的id
//        nMsgId --起始拉取msg的id  ： nMsgCnt--拉取的msg的数目
//        lsMsg  --存放结果的容器 
                               ...
                               uint32_t nMsgCnt, list<IM::BaseDefine::MsgInfo>& lsMsg)
{
  ...
  if (nRelateId != INVALID_VALUE){
      ...
      if (pDBConn){
          //有8张message table，msg具体存储在哪张table跟relationId有关
         ...
         if (pResultSet){
             while (pResultSet->Next()){
                 ...
                 cMsg.set_from_session_id(pResultSet->GetInt("fromId"));  //消息发送方的id
                 ...
                 if(IM::BaseDefine::MsgType_IsValid(nMsgType)){
                     ...
                     log("invalid msgType. userId=%u, peerId=%u, msgId=%u, msgCnt=%u, msgType=%u", 
        ...
        pDBManager->RelDBConn(pDBConn); //该链接应该在获取得到结果后就释放？？？
        if (!lsMsg.empty())
            CAudioModel::getInstance()->readAudios(lsMsg);  //如果是语音消息，将从这里获取语音地址。
      }
      else
...
bool CMessageModel::sendMessage(uint32_t nRelateId, uint32_t nFromId, uint32_t nToId, IM::BaseDefine::MsgType nMsgType, uint32_t nCreateTime, uint32_t nMsgId, string& strMsgContent)
{
  ...
  if (pDBConn){
      ...
      string strSql = "insert into " + strTableName 
                      + " (`relateId`, `fromId`, `toId`, `msgId`, `content`, `status`, `type`, `created`, `updated`) values(?, ?, ?, ?, ?, ?, ?, ?, ?)";
      // a.必须在释放连接前delete CPrepareStatement对象，否则有可能多个线程操作mysql对象，会crash
      // @1.要想mysql对象线程安全，应该在其获取接口实现。而不是由用户实现？？？
      //    即使做到上面的a点，mysql还是不安全的，因为sendMessage可以由多个线程同时调用。 
...
void CMessageModel::getLastMsg(uint32_t nFromId, uint32_t nToId, 
                               ...
                               IM::BaseDefine::MsgType& nMsgType, uint32_t nStatus)
{
  ...
  if (nRelateId != INVALID_VALUE){
      ...
      if (pDBConn){
          ...
          if (pResultSet){
              ...
              log("no result set: %s", strSql.c_str());
          pDBManager->RelDBConn(pDBConn);  //该连接应该在上面获得结果后，就释放？？？
      }
      else
...
void CMessageModel::getMsgByMsgId(uint32_t nUserId, uint32_t nPeerId, 
                                  const list<uint32_t> &lsMsgId, list<IM::BaseDefine::MsgInfo> &lsMsg)
{
  if(lsMsgId.empty())
      return ;
  uint32_t nRelateId = CRelationModel::getInstance()->getRelationId(nUserId, nPeerId, false);
