...
CImUser::CImUser(string& user_name) //string -> string&
{
  //log("ImUser, userId=%u\n", user_id);
...
CImUser* CImUserManager::GetImUserByLoginName(string login_name) //string -> string&
{
  CImUser* pUser = NULL;
...
bool CImUserManager::AddImUserByLoginName(stringi& login_name, 
                                          CImUser *pUser) //string -> string&
{
  bool bRet = false;
  //todo:该函数会在多线程被调用，应该加锁保护。
...
void CImUserManager::RemoveImUserByLoginName(string& login_name)  //string -> string&
{
...
{
  CImUser* pImUser = NULL;
  for (ImUserMap_t::iterator it = m_im_user_map.begin(); 
       it != m_im_user_map.end(); it++){
      pImUser = (CImUser*)it->second;
      if (pImUser->IsValidate()){
