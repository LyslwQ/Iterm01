...
void CMsgConn::_HandleLoginRequest(CImPdu* pPdu)
{
  ...
  if (result) {
    ...
    pdu.SetSeqNum(pPdu->GetSeqNum());
    pDbConn->SendPdu(&pdu);  //发送给db_proxy_server，验证密码
}
...
void CMsgConn::_HandleClientGetMsgListRequest(CImPdu *pPdu)
{
  ...
  log("HandleClientGetMsgListRequest, req_id=%u, session_type=%u, 
  ...
  if (pDBConn) {
      ...
      pPdu->SetPBMsg(&msg);
      pDBConn->SendPdu(pPdu);  //发送数据给db_proxy_server拉数据。
  }
}
...
void CMsgConn::_HandleClientMsgReadAck(CImPdu* pPdu)
{
   ...
   uint32_t session_id = msg.session_id();
   uint32_t msg_id = msg.msg_id();
   log("HandleClientMsgReadAck, user_id=%u, session_id=%u, msg_id=%u, session_type=%u. ", 
   ...
   //通过routeServer通知其他msgServer上的用户。
   CRouteServConn* pRouteConn = get_route_serv_conn();
   if (pRouteConn) 
       pRouteConn->SendPdu(&pdu);
...
void CMsgConn::_HandleClientGetLatestMsgIDReq(CImPdu *pPdu)
{
  ...
  uint32_t session_type = msg.session_type();
  uint32_t session_id = msg.session_id();
  log("HandleClientGetMsgListRequest, user_id=%u, session_id=%u, session_type=%u. ", 
